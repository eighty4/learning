...
public class FunImportingDynamicModule {
    ...
    public static class ApiFileSystem implements FileSystem {
        ...
        public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>... attrs) throws IOException {
            if (path.toString().equals("api")) {
                return new ByteArrayChannel("""
                        export const Service = {getFoo: () => 'foo'}
                        export const Util = {addBar: (foo) => `${foo}bar`}
                        """.getBytes(), true);
            } else {
                throw new UnsupportedOperationException();
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (var context = Context.newBuilder("js")
                ...
                .allowIO(true)
                .fileSystem(new ApiFileSystem())
                .build()) {

            var result = context.eval(sourceFromString("""
                    import {Service, Util} from 'api'
                    export default Util.addBar(Service.getFoo())
                    """));

            assert(result.getMember("default").asString().equals("foobar"));

            System.out.printf("default=%s\n", result.getMember("default").asString());
        }
    }
}
